...
import { AlertCircle, Trophy, Target, Check, CheckCheck, Zap } from 'lucide-react';
...
export function ClientActivity({ token }: ClientActivityProps) {
  ...
  const fetchNotifications = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-d58ce8ef/client/notifications`,
        {
          headers: {
  ...
  const markAsRead = async (notificationId: string) => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-d58ce8ef/client/notifications/${notificationId}/read`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const markAllAsRead = async () => {
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-d58ce8ef/client/notifications/read-all`,
        {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${token}`,
    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <div className="bg-primary text-primary-foreground px-6 py-8 sticky top-0 z-10">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-primary-foreground mb-1">Activity</h1>
